...
def saveData():
    ...
    ########## creates new file ##########
    while True:
        if os.path.exists(f"databron/saveFile.txt"):
            if os.path.exists(f"databron/saveFile{fileNumber}.txt"):
                fileNumber += 1
            else:
                with open(f"databron/saveFile{fileNumber}.txt","w") as file:
                    break
        else:
            with open(f"databron/saveFile.txt","w") as file:
                    break
    #################
...
NameQuestion.pack()
continueButton = ttk.Button(text="Continue",command=NextQuestion)
continueButton.pack()
survey.mainloop()
